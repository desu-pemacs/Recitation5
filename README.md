...
This recitation requires that you work in pairs to reimplement a Circle class.  The class uses the Point class developed in last week's sessions.

## Implementing a Circle Class
A circle is an object that can be described by its location and radius.  Depending on the application, a circle may have other properties like color, line width, fill color, etc...  For the purpose of this recitation, it is sufficient to describe our circle using the location and radius.  As such, our Circle class must include two fields called location and radius.  Unlike the last recitation exercise, we will use the Point class that was developed in class last week.
...
## Using the Circle Class - Recitation5 Class
